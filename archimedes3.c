...
real mstar2;
real BKTQ,QH;
real SMH[NOAMTIA+1][4];
real HHM[NOAMTIA+1][4];
real HM[NOAMTIA+1][4];
real GM[NOAMTIA+1];
real gm[NOAMTIA+1][5][3];
real SWK[NOAMTIA+1][5][14][DIME+1];
real SWK_N[NOAMTIA+1][5][14][DIME+1];
real Scat_Rate[NOAMTIA+1][5][14][DIME+1];
real Scat_Rate_N[NOAMTIA+1][5][14][DIME+1];
real P[NPMAX+1][7];
real KX,KY,KZ,X,Y;
real TS,EPP,DDmax;
...
real EPSR[NOAMTIA+1];
real EPF[NOAMTIA+1];
real MSTAR[NOAMTIA+1][6][5];
real alphaK[NOAMTIA+1][6];
real EG[NOAMTIA+1];
real HWO[NOAMTIA+1][7];
real DTK[NOAMTIA+1][7];
real ZF[NOAMTIA+1][7];
real RHO[NOAMTIA+1];
real DA[NOAMTIA+1][5];
real UL[NOAMTIA+1];
real EMIN[NOAMTIA+1][5];
real XVAL[NOAMTIA+1];
...
// For the others, see comments below
     ...
     NOVALLEY[GERMANIUM]=3;  // L,G,X-valley
...
// Intervalley Phonon Deformational Potential (eV)
    DIV[0]=3.0e10 * Q;
    DIV[1]=2.0e9 * Q;
    DIV[2]=2.0e10 * Q;
    DIV[3]=4.06e10 * Q;
...
// Acoustic deformation potential (Joule)
     DA[SILICON][0]=9.*Q;      // Fischetti -- Jacoboni Reggiani
     DA[GERMANIUM][0]=11.*Q;    // Fischetti -- Jacoboni Reggiani
     DA[GERMANIUM][1]=5.*Q;    // Fischetti -- Jacoboni Reggiani
     DA[GERMANIUM][2]=9.*Q;    // Fischetti -- Jacoboni Reggiani
     DA[GAAS][0]=7.*Q;         // Fischetti - Gamma valley
     DA[INSB][0]=7.*Q;         // Fischetti
     DA[ALSB][0]=4.6*Q;        // Fischetti
     DA[ALAS][0]=9.3*Q;        // Fischetti
     DA[ALP][0]=9.3*Q;         // Fischetti
     DA[GAP][0]=7.4*Q;         // Fischetti
     DA[GASB][0]=9.*Q;         // Fischetti
     DA[INAS][0]=8.2*Q;        // Fischetti
     DA[INP][0]=6.2*Q;         // Fischetti
...
// eventual second valley
     EMIN[GAAS][2]=0.323;
     EMIN[GERMANIUM][2]=0.055;  // G-Valley
     EMIN[GERMANIUM][3]=0.18357;  // D-Valley

// Definition of effective mass for all materials in all valleys
     MSTAR[SILICON][1][0]=0.32;     // see Sellier, Tomizawa, etc.
     MSTAR[GAAS][1][0]=0.067;       // Gamma-valley -- see Tomizawa
     MSTAR[GAAS][2][0]=0.350;       // L-valley     -- see Tomizawa
     MSTAR[GERMANIUM][1][0]=0.1186;   // L valley Couductivity Mass
     MSTAR[GERMANIUM][1][1]=0.1547;   // L valley Density Mass
     MSTAR[GERMANIUM][2][0]=0.1797;   // Gamma valley Conductivity Mass
     MSTAR[GERMANIUM][2][1]=0.1797;   // Gamma valley Density Mass
     MSTAR[GERMANIUM][3][0]=0.2061;   // Delta valley Conductivity Mass
     MSTAR[GERMANIUM][3][1]=0.4969;   // Delta valley Density Mass
     ...
     MSTAR[ALP][1][0]=0.22;         // Gamma-valley -- see Ram-Mohan
     MSTAR[GAP][1][0]=0.13;         // Gamma-valley -- see Ram-Mohan J.App.Phys. Vol.89, Num.11
     MSTAR[GASB][1][0]=0.039;       // Gamma-valley -- see Ram-Mohan
     MSTAR[INAS][1][0]=0.026;       // Gamma-valley -- see Ram-Mohan J.App.Phys. Vol.89, Num.11
     MSTAR[INP][1][0]=0.0795;       // Gamma-valley -- see Ram-Mohan
// non-parabolicity coefficients
     ...
     alphaK[GERMANIUM][2]=0.3;  // Gamma valley - Jacoboni Reggiani
     alphaK[GERMANIUM][3]=0.3;  // Delta valley - Jacoboni Reggiani
// Lattice constants
...
// semiconductor compounds acoustic deformation potential (Joule)
     DA[ALXINXSB][0]=XVAL[ALXINXSB]*(DA[ALSB][0]+DA[INSB][0]);
     DA[ALXIN1XSB][0]=XVAL[ALXIN1XSB]*DA[ALSB][0]+(1.-XVAL[ALXIN1XSB])*DA[INSB][0];
     DA[INXGA1XAS][0]=XVAL[INXGA1XAS]*DA[INAS][0]+(1.-XVAL[INXGA1XAS])*DA[GAAS][0];
     DA[INXAL1XAS][0]=XVAL[INXAL1XAS]*DA[INAS][0]+(1.-XVAL[INXAL1XAS])*DA[ALAS][0];
     DA[INXGAXXAS][0]=XVAL[INXGAXXAS]*DA[INAS][0]+XVAL[INXGAXXAS]*DA[GAAS][0];
...
// GAMMA-valley effective mass
     MSTAR[ALXINXSB][1][0]=XVAL[ALXINXSB]*(MSTAR[ALSB][1][0]+MSTAR[INSB][1][0]);
     MSTAR[ALXIN1XSB][1][0]=XVAL[ALXIN1XSB]*MSTAR[ALSB][1][0]+(1.-XVAL[ALXIN1XSB])*MSTAR[INSB][1][0];
     MSTAR[INXGA1XAS][1][0]=XVAL[INXGA1XAS]*MSTAR[INAS][1][0]+(1.-XVAL[INXGA1XAS])*MSTAR[GAAS][1][0];
     MSTAR[INXAL1XAS][1][0]=XVAL[INXAL1XAS]*MSTAR[INAS][1][0]+(1.-XVAL[INXAL1XAS])*MSTAR[ALAS][1][0];
     MSTAR[INXGAXXAS][1][0]=XVAL[INXGAXXAS]*MSTAR[INAS][1][0]+XVAL[INXGAXXAS]*MSTAR[GAAS][1][0];
// L-valley effective mass
     MSTAR[ALXINXSB][2][0]=XVAL[ALXINXSB]*(MSTAR[ALSB][2][0]+MSTAR[INSB][2][0]);
     MSTAR[ALXIN1XSB][2][0]=XVAL[ALXIN1XSB]*MSTAR[ALSB][2][0]+(1.-XVAL[ALXIN1XSB])*MSTAR[INSB][2][0];
     MSTAR[INXGA1XAS][2][0]=XVAL[INXGA1XAS]*MSTAR[INAS][2][0]+(1.-XVAL[INXGA1XAS])*MSTAR[GAAS][2][0];
     MSTAR[INXAL1XAS][2][0]=XVAL[INXAL1XAS]*MSTAR[INAS][2][0]+(1.-XVAL[INXAL1XAS])*MSTAR[ALAS][2][0];
     MSTAR[INXGAXXAS][2][0]=XVAL[INXGAXXAS]*MSTAR[INAS][2][0]+XVAL[INXGAXXAS]*MSTAR[GAAS][2][0];
