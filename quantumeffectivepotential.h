...
// =============================
 if(QEP_MODEL==QEP_CALIBRATED_BOHM || QEP_MODEL==QEP_BOHM){
  ...
  for(i=3;i<=nx-1;i++)
    for(j=3;j<=ny-1;j++){
     if(n_x[i][j]>0.0){
      tmp=((MSTAR[i_dom[i+1][j]][1][0]-MSTAR[i_dom[i][j]][1][0])/(MSTAR[i_dom[i+1][j]][1][0]*MSTAR[i_dom[i][j]][1][0]*M))
                *E*pow(n_x[i][j],D)*(n_x[i+1][j]-n_x[i][j])*odx2
               +((MSTAR[i_dom[i][j+1]][1][0]-MSTAR[i_dom[i][j]][1][0])/(MSTAR[i_dom[i][j+1]][1][0]*MSTAR[i_dom[i][j]][1][0]*M))
...
//  real tmp;
  ...
  for(i=2;i<=nx;i++)
   for(j=2;j<=ny;j++){
    // electron wavelenght
    alpha2=HBAR*HBAR/(8.*M*MSTAR[i_dom[i][j]][1][0]*KB*TL);
