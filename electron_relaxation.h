 ...
 for(c=1;c<=50;c++)
  for(i=ND;i<=nx+ND;i++)
   for(j=ND;j<=ny+ND;j++){
     ...
     ktaup=M*MSTAR[i_dom[i][j]][1][0]*MIU0*TL/Q;
     ktauw=MIU0*TL*KB/(Q*VS*VS);
     taup=ktaup/t;
     u2d[i][j][4]+=-DT/50.*(Q*(u2d[i][j][2]*E[i-1][j-1][0]
                  +u2d[i][j][3]*E[i-1][j-1][1])
                  +u2d[i][j][1]*1.5*KB*(t-TL)/tauwi(1.5*KB*t));
     u2d[i][j][2]+=-DT/50.*(u2d[i][j][1]*Q*E[i-1][j-1][0]/(M*MSTAR[i_dom[i][j]][1][0])
                  +u2d[i][j][2]/taup);
     u2d[i][j][3]+=-DT/50.*(u2d[i][j][1]*Q*E[i-1][j-1][1]/(M*MSTAR[i_dom[i][j]][1][0])
                  +u2d[i][j][3]/taup);
   }
// DT*=2.;
}
